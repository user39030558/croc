...
func (c *Client) prepareFile(index int, algorithm string, scratch *filePreparationScratch) (cacheHit bool, err error) {
	if index < 0 || index >= len(c.FilesToTransfer) {
		return false, fmt.Errorf("invalid file preparation index %d", index)
	}
	...
	hash, cacheHit, err := c.hashSourceFile(fullPath, algorithm, fileInfo.Size > 1e7)
	if err != nil {
		return false, err
	}
	...
	if !sourceInfoMatches(fileInfo, before, after) {
		return false, fmt.Errorf("source changed while preparing %s", fullPath)
	}
...
func (c *Client) prepareAllFiles(algorithm string, force bool) error {
	...
	for i := range c.FilesToTransfer {
		...
		if _, err := c.prepareFile(i, algorithm, &scratch); err != nil {
			return err
		}
	}
	return nil
	...
	c.remainingPreparationOnce.Do(func() {
		go func() {
			...
			for i := range c.FilesToTransfer {
				...
				if _, err := c.prepareFile(i, c.preparedHashAlgorithm, &scratch); err != nil {
